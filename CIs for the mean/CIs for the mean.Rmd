...
To make the code reusable, we encapsulate each procedure in certain R functions—``ci_mean_known_sigma`` and ``ci_mean_unknown_sigma_2``—whose arguments reflect the relevant parameters. We then apply these functions to a few problems from [@dekking2005modern], to which we refer for the theoretical background; see also [@delima2025probab] for further discussion.

We also include a comparison of these intervals in the special case where the sample standard deviation used in the t-interval is assumed to coincide exactly with the population standard deviation required for the corresponding z-interval. As a final theoretical bonus, we construct a confidence interval for this estimated standard deviation itself, using two-sided quantiles of the chi distribution ($\chi_{n-1}$), the square root of chi-square distribution ($\chi_k^2$).
...
\]
We have $\mathbb E(X)=\mu$ and ${\rm var}(X)=\sigma^2$, so the shape of $\psi_{(\mu,\sigma^2)}$, as a member of this family of densities, is completely determined by these population parameters. 
...
\[
P\left(-z_{1-\alpha/2}\leq \overline Z_n\leq z_{1-\alpha/2} \right)=1-\alpha,
\]
where
\[
z_{1-\alpha/2}=\Phi^{-1}\left(1-\frac{\alpha}{2}\right), \quad \Phi(x)=\int_{-\infty}^x\psi_{(0,1)}(t)dt
\]
...
\[
\mu\in\left[\overline X_n\mp z_{1-\alpha/2}\frac{\sigma}{\sqrt{n}}\right]\,\,\,\textrm{with prob.}=1-\alpha.
\]
...
#### The first problem (taken from [@dekking2005modern], page 347)

![](coal.jpg)

Since $\sigma=0.1$ is known, we may implement the theoretical formula for the z-interval above to check the computation in the end of the example. 
...
ci_mean_known_sigma <- function(x, sigma, alpha = 0.05) {
  ...
  se <- sigma / sqrt(n)
  # normal critical value (quantile)
...
In particular, $S_n\stackrel{p}{\to}\sigma$  so that $S_n$ is **consistent** for the standard deviation $\sigma$ and hence qualifies as an estimator for $\sigma$. It follows  that the **studentization**
\[
...
\[
\mu\in\left[\overline X_n\mp z_{1-\alpha/2}\frac{S_n}{\sqrt{n}}\right]\,\,\,\textrm{with prob.}\approx 1-\alpha,
\]
...
\[
\mu\in\left[\overline X_n\mp t_{n-1,1-\alpha/2}\frac{S_n}{\sqrt{n}}\right]\,\,\,\textrm{with prob.}= 1-\alpha,
\]
...
ci_mean_unknown_sigma_2 <- function(y, alpha = 0.05) {
  ...
  # Student t critical value (quantile)
...
... to check that the z-interval is a bit smaller. This simply confirms that the Student t-distribution has heavier tails than the normal distribution, which implies  $t_{n-1,1-\alpha/2}>z_{1-\alpha/2}$ for any choice of $\alpha$. Since $t_{n-1,1-\alpha/2}\to z_{1-\alpha/2}$ as $n\to+\infty$, the distinction between the two intervals becomes negligible for large sample sizes; it is only for relatively small $n$ (such as $n=23$) that the difference becomes appreciable.

#### A confidence interval for the variance

In the previous paragraph we used the estimated value for $\sigma$ in order to compare the two kinds of intervals, but the question remains: how good is this approximation? To check this, we need a confidence interval for $\sigma$, the population standard deviation around the point estimation $\sigma=1.78$.

#### A glimpse at the theory III

We will make use of an already mentioned fact:

- If $X_j\sim\mathcal N(\mu,\sigma^2)$ then $\frac{n-1}{\sigma^2}S_n^2\sim\chi^2_{n-1}$, the **chi-square** distribution of $n-1$ degrees of freedom.

We recall that if $X\sim\chi^2_k$, the **chi-square distribution** with $k\geq 1$ degrees of freedom, then $dP_X=\psi_Xdx$, with the corresponding pdf $\psi_X$ being given by 
\[
\psi_X(x)=\frac{1}{2^{k/2}\Gamma(k/2)}x^{{\frac{k}{2}-1}}e^{-x/2}{\bf 1}_{(0,+\infty)}(x), \quad x\in\mathbb R.
\]
It follows from [@delima2025probab], Remark 2.16, that $Y=\sqrt{X}\sim\chi_k$, 
the **chi distribution** with $k\geq 1$ degrees of freedom, where $dP_Y=\psi_Ydx$, with the corresponding pdf $\psi_Y$ being given by 
\[
\psi_Y(x)=\frac{1}{2^{\frac{k}{2}-1}\Gamma(k/2)}x^{k-1}e^{-x^2/2}{\bf 1}_{(0,+\infty)}(x), \quad x\in\mathbb R.
\]
Hence, if 
\[
F_k(x)=\int_0^x\psi_Y(t)dt, \quad x>0,
\]
is the associated cdf we define, for any $0<\alpha<1/2$, the corresponding **quantiles** 
\[
\chi_{k,\alpha/2}=F_k^{-1}(\alpha/2)< \chi_{k,1-\alpha/2}=F_k^{-1}(1-\alpha/2).
\]
As can be easily checked, 
\[
\chi_{k,\delta}=\sqrt{\chi_{k,\delta}^2}\,\,\,\,\textrm{for any}\,\,\,\,\delta,
\]
where $\chi_{k,\delta}^2$  is the  corresponding quantile for the chi-square.

With this notation (for $k=n-1$), and defining 
\[
{\mathscr S}_n=\sqrt{n-1}S_n,
\]
it follows from the fact above that
\[
P\left(\frac{\mathscr S_n}{\chi_{n-1,1-\alpha/2}}\leq \sigma\leq \frac{\mathscr S_n}{\chi_{n-1,\alpha/2}}\right)=
P\left(\chi_{n-1,\alpha/2}\leq\frac{\mathscr S_n}{\sigma}\leq \chi_{n-1,1-\alpha/2}\right)=1-\alpha, 
\]
which can be rewritten as a confidence interval for $\sigma$:
\[
\sigma\in\left[\frac{\mathscr S_n}{\chi_{n-1,1-\alpha/2}},\frac{\mathscr S_n}{\chi_{n-1,\alpha/2}}\right]\,\,\,\textrm{with prob.}= 1-\alpha
\]



#### Constructing a confidence interval around the estimated standard deviation

We may now easily implement the confidence interval above ...
...
ci_sd <- function(y, alpha = 0.05) {
  ...
  # here we determine the chi critical values, or quantiles, using that they may be computed from the chi-squared critical values, which are available in R, by taking a square root
  chi_crit_lower <- sqrt(qchisq(alpha/2,df=m-1))
  chi_crit_upper <- sqrt(qchisq(1-alpha/2,df=m-1))
  # finally, the interval endpoints
  chi_lower <- scr_sd / chi_crit_upper
  chi_upper <- scr_sd / chi_crit_lower
  # return as a list
  return(list(
    m        = m,
    sd_hat   = sd_hat_2,
...
# evaluation of the function at a 95% confidence level
...
chi_ci_table <- data.frame(
  Method        = c("Chi-interval for sigma at 95%"),
  CI_Lower      = round(c(ci_sd_95$lower), 4),
  Estimation    = round(c(ci_sd_95$sd_hat), 4),
  CI_Upper      = round(c(ci_sd_95$upper), 4)
)
knitr::kable(chi_ci_table,caption = "Chi-interval for the estimated standard deviation")
```

We therefore see that the resulting confidence interval is somewhat conservative, and the assumption underlying the comparison above should consequently be taken with a grain of salt.
