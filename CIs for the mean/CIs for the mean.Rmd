...
To make the code reusable, we encapsulate each procedure in an R function—``ci_mean_known_sigma`` and ``ci_mean_unknown_sigma_2``—whose arguments reflect the relevant parameters. We then apply these functions to a few problems from [@dekking2005modern], to which we refer for the theoretical background; see also [@delima2025probab] for further discussion.

We also include a comparison of the two types of intervals in the special case where the sample standard deviation used in the t-interval is assumed to coincide exactly with the population standard deviation required for the corresponding z-interval. As a final theoretical bonus, we construct a confidence interval for this estimated standard deviation itself, using two-sided quantiles of the chi distribution ($\chi_{n-1}$).
