...
plot_theoretical_distributions <- function() {
  ...
  p_cdf <- ggplot(curve_data, aes(x = x, y = cdf_y)) +
    ...
    theme_minimal() +
    theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
  # Return both plot objects in a list
   return(list(
    pdf_plots = p_pdf,
    cdf_plots = p_cdf
  ))
...
The three distinct shapes of the starting distributions are now apparent from the PDFs plots (in red):
...
We now look at three more interactions ...
...
We will run the simulation only for two (extreme) values of $n$. First, $n=5$:
...
Note that the shape factor ($\varepsilon$) for **uniform** is much smaller that that for **exponential**, thus denouncing the pronounced skewness of this latter distribution. In particular, this indicates that the convergence toward a normal for **exponential** should be much slower.  
...
In conclusion, this final visualization certainly illustrates **Berry-Esseen 
theorem** in action.
